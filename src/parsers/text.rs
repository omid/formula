...
impl Formula<'_> {
    pub(crate) fn parse_left(pair: Pair<Rule>) -> Result<Expr> {
        ...
        let text = Self::parse_pair(args.next().ok_or(Error::Parser(RULE))?)?;
        let num_chars = args.next().map(Self::parse_pair).unwrap_or(Ok(Expr::Number(1.0)))?;
    ...
    pub(crate) fn parse_leftb(pair: Pair<Rule>) -> Result<Expr> {
        ...
        let mut args = pair.into_inner();
        let text = Self::parse_pair(args.next().ok_or(Error::Parser(RULE))?)?;
        let num_bytes = args.next().map(Self::parse_pair).unwrap_or(Ok(Expr::Number(1.0)))?;
    ...
    pub(crate) fn parse_mid(pair: Pair<Rule>) -> Result<Expr> {
        ...
        let mut args = pair.into_inner();
        let text = Self::parse_pair(args.next().ok_or(Error::Parser(RULE))?)?;
        let start = Self::parse_pair(args.next().ok_or(Error::Parser(RULE))?)?;
        let len = Self::parse_pair(args.next().ok_or(Error::Parser(RULE))?)?;
    ...
    pub(crate) fn parse_char(pair: Pair<Rule>) -> Result<Expr> {
        const RULE: &str = "char";
        let mut args = pair.into_inner();
        let number = Self::parse_pair(args.next().ok_or(Error::Parser(RULE))?)?;
    ...
    pub(crate) fn parse_exact(pair: Pair<Rule>) -> Result<Expr> {
        ...
        let mut args = pair.into_inner();
        let text1 = Self::parse_pair(args.next().ok_or(Error::Parser(RULE))?)?;
        let text2 = Self::parse_pair(args.next().ok_or(Error::Parser(RULE))?)?;
    ...
    pub(crate) fn parse_find(pair: Pair<Rule>) -> Result<Expr> {
        const RULE: &str = "find";
        let mut args = pair.into_inner();
        let find_text = Self::parse_pair(args.next().ok_or(Error::Parser(RULE))?)?;
        let within_text = Self::parse_pair(args.next().ok_or(Error::Parser(RULE))?)?;
        let start_num = args.next().map(Self::parse_pair).unwrap_or(Ok(Expr::Number(1.0)))?;
    ...
    pub(crate) fn parse_search(pair: Pair<Rule>) -> Result<Expr> {
        ...
        let mut args = pair.into_inner();
        let search_text = Self::parse_pair(args.next().ok_or(Error::Parser(RULE))?)?;
        let within_text = Self::parse_pair(args.next().ok_or(Error::Parser(RULE))?)?;
        let start_num = args.next().map(Self::parse_pair).unwrap_or(Ok(Expr::Number(1.0)))?;
    ...
    pub(crate) fn parse_fixed(pair: Pair<Rule>) -> Result<Expr> {
        ...
        let mut args = pair.into_inner();
        let number = Self::parse_pair(args.next().ok_or(Error::Parser(RULE))?)?;
        let decimals = args.next().map(Self::parse_pair).unwrap_or(Ok(Expr::Number(2.0)))?;
        let no_commas = args.next().map(Self::parse_pair).unwrap_or(Ok(Expr::Bool(false)))?;
    ...
    pub(crate) fn parse_rept(pair: Pair<Rule>) -> Result<Expr> {
        ...
        let mut args = pair.into_inner();
        let text = Self::parse_pair(args.next().ok_or(Error::Parser(RULE))?)?;
        let rept = Self::parse_pair(args.next().ok_or(Error::Parser(RULE))?)?;
    ...
    pub(crate) fn parse_replace(pair: Pair<Rule>) -> Result<Expr> {
        ...
        let start = Self::parse_pair(args.next().ok_or(Error::Parser(RULE))?)?;
        let len = Self::parse_pair(args.next().ok_or(Error::Parser(RULE))?)?;
        let new_text = Self::parse_pair(args.next().ok_or(Error::Parser(RULE))?)?;
    ...
    pub(crate) fn parse_textjoin(pair: Pair<Rule>) -> Result<Expr> {
        ...
        let mut args = pair.into_inner();
        let delim = Self::parse_pair(args.next().ok_or(Error::Parser(RULE))?)?;
        let ignore_empty = Self::parse_pair(args.next().ok_or(Error::Parser(RULE))?)?;
        let text = Self::parse_pair(args.next().ok_or(Error::Parser(RULE))?)?;
        let texts = args.map(Self::parse_pair).collect::<Result<Vec<_>>>()?;
